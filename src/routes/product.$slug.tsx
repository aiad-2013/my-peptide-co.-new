...
function getStockLeft(id: string) { return Math.floor(seededRandom(id, 7) * 8) + 3; }
function getSoldThisWeek(id: string) { return Math.floor(seededRandom(id, 13) * 40) + 12; }
...
const ProductDetailContent = () => {
  const { slug } = Route.useParams();  // ← TanStack way (replaces useParams from react-router-dom)
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 mb-12">
          ...
          <div>
                ...
                <div className="mb-6 space-y-3">
                  <div>
                    ...
                    <div className="w-full h-px bg-border overflow-hidden">
                      <div className="h-full bg-accent/60 transition-all duration-700" style={{ width: `${stockPct}%` }} />
                    </div>
                  </div>
                  ...
                  {orderDeadline.isPastCutoff && (
                    <div className="flex items-center gap-2">
                      <Clock className="w-3.5 h-3.5 text-muted-foreground/50 flex-shrink-0" />
                      <p className="text-[11px] text-muted-foreground">Order now for next business day dispatch</p>
                    </div>
                  )}
                </div>
              );
            ...
            <div className="hidden md:block">
              ...
              <Button variant="gold" size="xl" className="w-full" onClick={handleAddToCart} disabled={!product.inStock}>
                <ShoppingCart className="w-5 h-5 mr-2" />
                Add to Cart — ${((product.salePrice || product.price) * quantity).toFixed(2)}
              </Button>
              {!product.inStock && (
                <p className="text-destructive text-sm text-center mt-3 font-medium">Currently out of stock</p>
              )}
            </div>
        ...
        <Tabs defaultValue="description" className="mb-16">
          <TabsList className="w-full justify-start border-b border-border rounded-none bg-transparent h-auto p-0 mb-8 gap-0">
            <TabsTrigger value="description" className="rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:text-accent px-6 py-3 text-sm font-medium text-muted-foreground data-[state=active]:shadow-none">
              Description
            </TabsTrigger>
            {product.faqs && product.faqs.length > 0 && (
              <TabsTrigger value="faq" className="rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:text-accent px-6 py-3 text-sm font-medium text-muted-foreground data-[state=active]:shadow-none">
                FAQ's
              </TabsTrigger>
            )}
            {reviews && reviews.length > 0 && (
              <TabsTrigger value="reviews" className="rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent data-[state=active]:text-accent px-6 py-3 text-sm font-medium text-muted-foreground data-[state=active]:shadow-none">
        ...
        {relatedProducts && relatedProducts.length > 0 && (
          <section className="pt-12 border-t border-border">
            <h2 className="text-2xl font-serif font-semibold text-foreground mb-8">Related Products</h2>
      ...
      >
        <div className="flex items-center border border-border rounded-md flex-shrink-0">
          ...
          <span className="px-3 py-2 font-medium min-w-[2.5rem] text-center text-sm">{quantity}</span>
          <button onClick={() => setQuantity(quantity + 1)} className="p-2.5 hover:bg-muted transition-colors">
            <Plus className="w-4 h-4" />
          </button>
        </div>
        <Button variant="gold" size="lg" className="flex-1" onClick={handleAddToCart} disabled={!product.inStock}>
